...
class ProtoShotXAI:
    ...
    def compute_score_from_features_localshot(self, features, iclass, k=3):
    ...
    def image_feature_attribution_localshot(self, support_data, query, class_indx, ref_pixel, pad=4, progress_bar=True):
        rows = np.shape(query)[1]
        cols = np.shape(query)[2]
        chnls = np.shape(query)[3]
