...
def calculate_dAAC(model, feature_attribution_map, input_image, query):
    ...
    model_outputs = normalize_list(model_outputs)
        # 计算dAAC
    dAAC = 1 - np.trapz(model_outputs, dx=1.0 / len(sorted_indices))
...
def plot_curves(insertion_curve, deletion_curve, save_path=None):
    ...
    plt.fill_between(x, deletion_curve, 1, where=(deletion_curve < 1), color='blue', alpha=0.1)
    plt.xlabel('Pixels Inserted')
    plt.grid(color='gray', linewidth=0.5, alpha=0.5)  # 添加灰色网格线
    y_values = [0, 0.2, 0.6, 0.4, 0.8, 1]
    for y_val in y_values:
...
if __name__ == '__main__':
    ...
    for dataset_str in ['birds', 'cars', 'dogs']:
        ...
        for i in range(class_num):
            protoshot_target_scores.append(protoshot_xai.image_feature_attribution(
        ...
        gradcam = GradCam(base_model.encoder, last_conv_layer_name, np.expand_dims(query, axis=0))
        for i in range(class_num):
            gradcam_target_scores.append(gradcam.make_gradcam_heatmap(np.expand_dims(episode_support_data[i], axis=0), pred_index=class_idx))
        for i in range(class_num):
