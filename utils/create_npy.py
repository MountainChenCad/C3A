...
def create_npy_dataset_omniglot(cars_root_dir, output_filename):
    # 定义路径
    data_dir = os.path.join(cars_root_dir, 'data')
    csv_path = os.path.join(cars_root_dir, 'splits', 'default', 'train.csv')
    ...
    # 初始化数据存储
    num_images = len(filenames)
    image_data = np.zeros((num_images, 28, 28, 1), dtype=np.uint8)

    # 处理并存储图像
    for filename in filenames:
        img_path = os.path.join(data_dir, filename)
        image_names = [f for f in os.listdir(img_path) if f.endswith('.png')]
        for i, image_name in enumerate(image_names):
            final_path = os.path.join(img_path, image_name)
            try:
                img = Image.open(final_path).convert('L')
                # img = img.resize((28, 28))
                image_data[i] = np.expand_dims(np.array(img), axis=-1)
            except Exception as e:
                print(f"Error processing {filename}: {str(e)}")
                # 如果出现错误，用全黑图像填充
                image_data[i] = np.zeros((28, 28, 1), dtype=np.uint8)
...
# 使用示例
create_npy_dataset_omniglot('../datasets/omniglot', '../data/omniglot_train_data.npy')